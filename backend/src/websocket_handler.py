...
async def handle_websocket_connection(websocket: WebSocket, session: ClientSession) -> None:
    """
    ...
    try:
        async for message in websocket.iter_text():
            try:
                ...
                await send_error_to_client(websocket, f"Message processing error: {str(e)}")
                
    except WebSocketDisconnect:
        logger.info(f"クライアントからのメッセージ受信終了: session_id={session.session_id}")
        raise
...
async def process_client_message(
    ...
    if message_type == RealtimeMessageType.AUDIO_APPEND.value:
        # 音声データを OpenAI に送信
        audio_data = data.get("audio", "")
        await realtime_client.send_audio(audio_data)
        
    elif message_type == RealtimeMessageType.AUDIO_COMMIT.value:
        # 音声入力完了をOpenAIに通知
        await realtime_client.commit_audio()
        
    elif message_type == RealtimeMessageType.CONVERSATION_INTERRUPT.value:
        # 会話中断をOpenAIに通知
        await realtime_client.interrupt_conversation()
        
    elif message_type == RealtimeMessageType.SESSION_UPDATE.value:
        # セッション設定更新
